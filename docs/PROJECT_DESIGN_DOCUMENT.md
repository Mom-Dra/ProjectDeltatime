...
- 2026-08-09 Pistol 장착 애니메이션 프로필의 손끝 기준축이 플레이어 루트 전방축에서 약 36.1도 어긋나는 수동 확인을 반영했다. `CharacterAnimationController`는 플레이어 입력 소스가 있고 현재 스타일이 `Pistol`일 때만 직렬화된 `pistolVisualYawOffset` 기본값 `+36.1°`를 시각 루트의 기본 및 대시 회전에 적용한다. 빈손·소총·근접 프로필과 적은 0°를 사용한다. 보정은 게임플레이 Rigidbody 루트, 이동·조준·발사 판정을 돌리지 않으며, 이후 `WeaponVisualPresenter.LateUpdate`의 기존 총구 조준 보정이 적용된다. **구현 완료**. Stage1 스모크는 Pistol만 36.1도이고 나머지 프로필은 0도인지 검사하도록 확장했으나, Unity Play Mode 실행은 **미실행**이다. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Visuals/CharacterAnimationController.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/Stage1CharacterAnimationPlayModeSmokeTest.cs`.
...
- 2026-08-09 `Pistol.asset`의 Tactical Pistol 손 모델은 보정 씬에서 확인한 로컬 위치 `(0.058, -0.009, -0.007)`, 회전 `(-11.904, 73.839, 185.269)`, 균일 스케일 `(0.65, 0.65, 0.65)`을 사용한다. 실제 발사 `Weapon Muzzle`은 모델 내부 로컬 위치 `(0, 0.112, 0.42)`, 회전 `(0, 0, 0)`이다. **구현 완료**. Unity 정적 검증은 통과했으며, 최종 시각적 손 그립·총구 축 체감은 **확인 불가**다. 근거: `ProjectDeltatime/Assets/_Project/Pistol.asset`, `ProjectDeltatime/Assets/_Project/Scripts/Visuals/WeaponVisualPresenter.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/WeaponCalibrationSceneBuilder.cs`.
...
|---|---:|---|---|
| 2026-08-09 | 1.6.12 | 플레이어 Pistol 애니메이션 프로필의 시각 루트에만 +36.1도 Y축 보정을 적용하고, 빈손·소총·근접·적에는 0도를 유지 | 플레이어 Pistol 손끝 기준축, 구르기 시각, Stage1 애니메이션 스모크 |
