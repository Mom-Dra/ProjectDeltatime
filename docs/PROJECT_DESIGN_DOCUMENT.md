...
- 2026-08-17 플레이어가 기존 상호작용 거리 안에서 실제로 무기를 획득·교체·공중 캐치할 수 있을 때만 IMGUI `E` 키캡 프롬프트를 표시한다. `PlayerCombat`은 공중 무기를 바닥 무기보다 우선해 `무기 캐치`로 판정하고, 바닥 무기는 빈손이면 `무기 줍기`, 장비 중이면 `무기 교체`로 구분한다. 사망, 전투 비활성, 하드 프리즈, `DEADLINE` 활성·해제 프레임, 일반 스테이지의 리플레이·클리어·사망 상태에는 숨긴다. 일반 `GameHud`는 하단 조작 안내보다 16px 위 중앙에, `TutorialHud`는 하단 진행 패널보다 20px 위 중앙에 같은 어두운 패널·청록 테두리·`E` 키캡을 표시한다. 입력 바인딩, 바닥 `1.25m`·공중 `1.15m` 범위, 캐치 버퍼와 획득/교환 동작은 변경하지 않았다. **구현 완료**. Unity 6000.1.13f1 컴파일 및 새 EditMode 상태 정책 테스트 5/5는 통과했다. Tutorial/Stage2 기존 PlayMode 스모크는 각각 Synty 프리팹 수 `216/262` 불일치, 투척·리플레이 기존 검증 실패로 완료되지 않아 이번 UI의 런타임 회귀 결과는 **확인 불가**다. 배치 Editor에서는 Game View 육안 캡처를 수행하지 않아 16:9 배치·가독성은 **미실행/확인 불가**다. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerCombat.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerCombatSubsystems.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/GameHud.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Tutorial/TutorialHud.cs`, `ProjectDeltatime/Assets/_Project/Tests/EditMode/CoreBehaviorTests.cs`, `ProjectDeltatime/WeaponInteractionPromptCompile.log`, `ProjectDeltatime/WeaponInteractionPromptEditMode.log`.
...
- **현재 동작 방식:** `MainScene`은 Canvas 기반 타이틀 화면이다. Canvas는 `1920×1080` 기준 `Scale With Screen Size`·폭/높이 일치값 `0.5`를 사용하고, 배경은 원본 이미지 비율을 유지한 채 화면을 덮는다. 로고는 좌측 상단, 배경 없는 흰색 `PLAY` 텍스트는 그 아래 좌측 고정 앵커에 배치되며 투명 입력 영역을 통해 `Tutorial`을 로드한다. 게임플레이 씬은 계속 런타임 IMGUI로 텍스트 패널과 진행 막대를 직접 그린다. 무기 상호작용이 가능한 동안에는 `E` 키캡과 `무기 줍기`·`무기 교체`·`무기 캐치` 중 현재 행동을 하단 중앙에 표시한다. 월드가 느릴수록 화면에 어두운 오버레이를 적용하며, 리플레이에서는 재생 시간과 `N`·`R` 조작만 표시한다.
...
- 일반 스테이지는 실제 상호작용 가능 시 하단 조작 안내보다 16px 위 중앙에 `E` 키캡·`무기 줍기`/`무기 교체`/`무기 캐치` 프롬프트를 표시한다. Tutorial은 하단 진행 패널보다 20px 위 중앙에 같은 프롬프트를 표시한다. **구현 완료**. 16:9 실제 가독성은 **확인 불가**다.
