...
- `WorldTimeVisualFeedback`가 런타임에 게임플레이 카메라의 `CombatFeedbackController`를 생성·연결한다. 발사는 한 번의 카메라 임펄스와 성공한 총기에만 0.07초 절차형 `MuzzleFlash`를 만들고, 실제 적중은 0.14초 확장 링·방사형 스파크 `HitFlash`와 무기별 `WorldTimeController.RequestHardFreeze`를 사용한다. `TopDownCameraController`는 추적 기준 위치·회전을 흔들림 출력과 분리하고 비스케일 감쇠·위치 0.16m/회전 0.5도 누적 상한을 적용하므로 흔들린 위치를 다음 추적 기준으로 사용하지 않는다. 전역 `Time.timeScale`과 물리적 플레이어 반동은 변경하지 않는다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Visuals/CombatFeedbackController.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Visuals/MuzzleFlash.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Utilities/HitFlash.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Player/TopDownCameraController.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Time/WorldTimeVisualFeedback.cs`.
- 무기별 직렬화 피드백 값은 권총 `0.045m/0.12도/0.09초/0.015초/0.18m`, 자동소총 `0.025m/0.08도/0.055초/히트스톱 없음/0.20m`, 샷건 `0.11m/0.32도/0.15초/0.045초/0.28m`, 근접 무기 `0.08m/0.24도/0.12초/0.05초/총구 플래시 없음` 순서로 위치 임펄스/회전/지속/히트스톱/총구 크기를 저장한다. 맨손은 `0.05m/0.15도/0.09초/0.03초`, 플레이어 피격은 `0.07m/0.20도/0.12초/0.04초` 런타임 기준을 사용한다. 기존 피해·탄약·사거리·플레이어 반동 값은 유지하고 재생성 경로도 같은 값을 적용한다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Pistol.asset`, `ProjectDeltatime/Assets/_Project/AutomaticRifle.asset`, `ProjectDeltatime/Assets/_Project/Shotgun.asset`, `ProjectDeltatime/Assets/_Project/MeleeWeapon.asset`, `ProjectDeltatime/Assets/_Project/Scripts/Combat/WeaponDefinition.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/PrototypeSceneBuilder.cs`.
- 적은 피격 즉시 `IsAlive=false`, AI·Collider·추가 피해 판정 비활성, 무기 드롭과 KILL/CLEAR 기록을 처리하되 시각 오브젝트는 비스케일 0.32초 동안 공격 방향으로 0.22m 이동하고 최대 12도 기울어진 뒤 제거된다. 마지막 킬은 이 0.32초 동안 리플레이 요청만 보류해 반응 전체를 녹화한다. `EnemyHealth`의 중복 HitFlash 생성은 제거하고 투사체·근접·투척 충돌 진입점이 충돌당 한 번만 생성한다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Enemies/EnemyHealth.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Level/StageController.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Combat/Projectile.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Combat/MeleeAttackResolver.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Combat/ThrownWeapon.cs`.
- `PlayerHealth.Damaged(DamageHit, bool)`는 실제 양수 피해에만 발생한다. 카메라 피드백은 0.18초 붉은 가장자리 비네트·피격 임펄스·하드 프리즈를 적용하며 대시 무적과 0 피해에서는 피드백을 만들지 않고 리플레이 진입 비활성화 때 즉시 끈다. 전투음 API는 선택적 `CombatFaction`을 받아 플레이어 관련 소리를 `spatialBlend 0.25/minDistance 10m`, 적 소리를 `1/2m`로 매 재생 초기화한다. 기존 호출은 기본 인수로 호환된다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerHealth.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Visuals/CombatFeedbackController.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Audio/SoundManager.cs`.
- Unity 6000.1.13f1 자체 C# 컴파일, `CoreBehaviorTests` 25/25, `CombatFeedbackTests` 4/4·`WorldTimeContractTests` 3/3·`ElevationFireAimTests` 3/3, SoundManager·Stage1 캐릭터 애니메이션·Replay PlayMode 스모크가 통과했다. `dotnet build ProjectDeltatime.sln`은 Unity가 생성한 기존 프로젝트가 삭제된 `Assets/TutorialInfo/Scripts/Readme.cs`와 설치되지 않은 패키지 어셈블리 참조를 유지해 실패했으나 Unity 자체 컴파일과 대상 테스트에서는 새 C# 오류가 없었다. 자동 검증은 **부분 구현**이며, 실제 Game View의 무기별 강도·청감·멀미 가능성은 **확인 불가**다. 근거: `ProjectDeltatime/Assets/_Project/Tests/PlayMode/CombatFeedbackTests.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/SoundManagerPlayModeSmokeTest.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/Stage1CharacterAnimationPlayModeSmokeTest.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/ReplayPlayModeSmokeTest.cs`, `ProjectDeltatime/CombatFeedbackEditMode.log`, `ProjectDeltatime/CombatFeedbackPlayMode.log`, `ProjectDeltatime/CombatFeedbackSoundSmoke.log`, `ProjectDeltatime/CombatFeedbackStage1AnimationSmoke.log`, `ProjectDeltatime/CombatFeedbackReplaySmoke.log`.
...
- MainScene은 기존 `background.png`·`titleLogo.png` 참조를 제거하고, ImageGen 내장 모드로 제작한 `mainMenuBackground.png`와 투명 `titleLogoWide.png`를 사용한다. 배경은 왼쪽 약 40%를 어두운 UI 여백으로 두고 오른쪽 주인공·붉은 탄환을 유지하며 총구 섬광과 시작 연기만 청록색으로 제한했다. 로고는 정확한 단일 행 `DELTA TIME`, 회백색 `DELTA`, 적색 `TIME`, 대각선 절단선과 약 4:1 가시 실루엣을 사용한다. 두 에셋은 Sprite/UI, sRGB, 알파 정책, 밉맵 비활성, Clamp, Bilinear, 최대 2048px로 임포트되며 기존 이미지는 복구용으로 보존한다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Image/mainMenuBackground.png`, `ProjectDeltatime/Assets/_Project/Image/titleLogoWide.png`, `ProjectDeltatime/Assets/_Project/Scenes/MainScene.unity`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/MainSceneBuilder.cs`.
- 실제 Unity 버튼 `START/OPTION/CREDITS/EXIT`는 선택 시 적색 반투명 바·경계·삼각형 포인터와 흰 글자를 표시하고 마우스 및 방향키/Enter 탐색을 지원한다. START와 저장된 `NextStage` 단축키는 Tutorial을 로드하며, Option/Credits/재바인딩 중에는 단축 시작을 차단한다. EXIT는 빌드 종료, 에디터 Play Mode 종료를 수행한다. `KoreanUiLocalizationBuilder`는 공용 Noto Sans KR 폰트만 유지하고 영문 START를 덮어쓰지 않는다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Scripts/UI/MainMenuController.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/MainMenuButtonFeedback.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/KoreanUiLocalizationBuilder.cs`.
- OPTION의 GRAPHICS는 해상도, Fullscreen Window/Windowed, Quality, VSync를 제공하고, KEYS는 이동 4방향·Fire·Throw·Dash·Deadline·Interact·Restart·Next Stage를 재바인딩하며, AUDIO는 Master/BGM/SFX 0~100%를 제공한다. 초안은 APPLY에서만 저장·적용하고 CANCEL은 폐기하며 RESET DEFAULTS는 초안만 초기화한다. Escape는 재바인딩 취소, 중복 키는 거부하고 이동과 일반 행동은 키보드, Fire/Throw는 키보드 또는 마우스 버튼으로 제한한다. `PlayerControlsFactory`가 저장 JSON을 런타임 입력에 적용하고 MainScene·EndingScene·GameHud·Tutorial·상호작용 키캡은 현재 바인딩을 표시한다. `SoundManager`는 기존 씬별 기본값과 DEADLINE 덕킹 위에 사용자 배율을 곱한다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Scripts/UI/GameSettingsService.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/MainMenuOptionsController.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Input/PlayerControlsFactory.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Audio/SoundManager.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/GameHud.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Tutorial/TutorialDirector.cs`.
- CREDITS는 Unity 6000.1.13f1, Synty POLYGON Generic/Nightclubs, Noto Sans KR OFL 1.1과 저장소에서 확인한 OpenGameArt·Kenney·Pixabay 오디오 출처를 영문 표시한다. EndingScene은 MainScene의 새 배경·로고는 유지하되 메뉴와 두 모달 계층을 제거하고 현재 Next Stage 바인딩을 복귀 안내에 표시한다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Scenes/EndingScene.unity`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/EndingSceneBuilder.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/EndingSceneController.cs`, `ProjectDeltatime/Assets/_Project/Audio/BGM/README.md`, `ProjectDeltatime/Assets/_Project/Audio/SFX/Combat/README.md`, `ProjectDeltatime/Assets/_Project/Audio/SFX/Deadline/README.md`.
- Unity 6000.1.13f1에서 MainScene 빌드·정적 검증 2회, 설정 EditMode, MainScene→EndingScene→MainScene→Tutorial PlayMode 스모크가 통과했다. 1920×1080, 1280×720, 2560×1080, 1024×768의 Main/GRAPHICS/KEYS/AUDIO/CREDITS 20개 PNG를 생성·크기 검증하고 기준·4:3·울트라와이드 캡처를 육안 확인해 로고·메뉴·주인공·모달 비중첩을 확인했다. **구현 완료** 검증이다. OS별 실제 빌드 해상도 전환과 플레이어가 직접 수행하는 전체 키/마우스 리바인딩 체감은 **확인 불가**로 남는다. 근거: `ProjectDeltatime/MainMenuBuild1.log`, `ProjectDeltatime/MainMenuBuild2Retry.log`, `ProjectDeltatime/MainMenuSettingsEditMode.log`, `ProjectDeltatime/MainMenuPlayModeSmoke.log`, `ProjectDeltatime/MainMenuVisualCaptureAllTabs.log`, `ProjectDeltatime/Logs/Validation/MainMenuCaptures`.

### 1.0.4 2026-08-18 총구 높이 기준 수평 발사 보정

- 수평 바닥·천장 클릭의 최종 발사 방향은 플레이어 루트가 아닌 실제 발사 원점, 즉 `WeaponController.Muzzle`의 Y를 기준으로 보정한다. `PlayerAim`은 수평 표면/Collider 없음 폴백인지 기록하고, `GetFireDirectionFrom(origin)`에서 그 경우에만 목표 Y를 전달받은 총구 `origin.y`로 바꾼다. 따라서 총구가 플레이어 Rigidbody보다 위·아래에 있어도 바닥을 클릭한 총알의 Y 방향은 정확히 0이다. 적 등 `IDamageable` Collider와 벽·경사면은 이 보정을 거치지 않아 기존 실제 3D 접점과 고저차 발사 방향을 유지한다. **구현 완료**. Unity 6000.1.13f1 배치 PlayMode `ElevationFireAimTests` 3/3이 총구와 플레이어 루트 높이가 다른 바닥 클릭·Collider 없음 폴백의 수평 방향, 위·아래 적 target의 3D 방향·명중, 숨은 전경 제외, 적 총기 상·하 방향을 통과했다. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerAim.cs`, `ProjectDeltatime/Assets/_Project/Tests/PlayMode/ElevationFireAimTests.cs`.

### 1.0.3 2026-08-18 바닥·천장 클릭 수평 발사 보정

- 3D 총기 조준의 최신 예외 규칙이다. 적 등 `IDamageable` Collider와 경사/수직 벽 표면은 실제 Ray 접점을 `FireAimPoint`로 써 총구에서 3D 직선 방향을 만든다. 반면 표면 법선의 절대 Y가 `0.7` 이상인 수평 바닥·천장 접점은 클릭한 X/Z를 유지하되 플레이어 Rigidbody의 현재 Y로 보정한다. 따라서 플레이어 바로 앞 바닥을 클릭해도 총구가 아래로 꺾이지 않으며, 위·아래 단상의 적을 정확히 클릭한 경우는 계속 실제 높이를 향한다. 플레이어 자신과 `ShadowsOnly` 전경, Collider가 전혀 없는 클릭의 기존 제외·폴백 규칙은 유지한다. **구현 완료**. Unity 6000.1.13f1 배치 PlayMode `ElevationFireAimTests` 3/3은 위·아래 `IDamageable` target의 3D 방향, 숨은 전경 건너뛰기, 수평 바닥 보정과 no-collider 수평 평면 폴백을 통과했다. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerAim.cs`, `ProjectDeltatime/Assets/_Project/Tests/PlayMode/ElevationFireAimTests.cs`.

### 1.0.2 2026-08-17 고저차 3D 총기 조준 기준

- 플레이어의 `PlayerAim`은 역할을 분리한다. `AimPoint`·`AimDirection`·`GetPlanarDirectionFrom`은 캐릭터 yaw 회전, 근접 공격, 투척, 카메라·HUD용으로 플레이어 현재 높이 수평 평면을 계속 사용한다. 반면 총기용 `FireAimPoint`는 마우스 카메라 Ray에서 가장 가까운 활성 비트리거 Collider의 실제 접점이며, `GetFireDirectionFrom(총구)`가 그 점까지의 전체 3D 방향을 제공한다. 플레이어 자신의 Collider와 부모 계층 Renderer가 `ShadowsOnly`인 전경 Collider는 건너뛰므로 Stage5 컷어웨이는 물리·시야 차단 성질을 유지하면서 발사 조준을 가로막지 않는다. 유효 Collider가 없으면 기존 수평 평면 교점으로 폴백한다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerAim.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/Stage5PlayModeSmokeTest.cs`.
- 플레이어 권총·자동소총·샷건의 일반 발사와 `DEADLINE` 예약 발사는 모두 총구 기준의 3D 발사 방향을 사용한다. 반동은 기존처럼 수평 성분만 사용하고, 무기 모델·캐릭터 회전은 yaw만 정렬한다. 근접 공격과 투척은 이번 규칙에서 제외돼 수평 조준을 유지한다. 적 총기도 총구에서 플레이어 루트의 실제 위치까지 3D 방향으로 발사하며, 경고선도 같은 3D 선분을 표시한다. `Projectile`의 직선 이동·SphereCast 충돌 구조는 이미 3D 방향을 처리하므로 중력, 탄도 곡선, 유도, 선행 조준, 모델 pitch는 추가하지 않았다. **구현 완료**. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerCombat.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Enemies/EnemyCombatant.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Combat/Projectile.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Visuals/WeaponVisualPresenter.cs`.
- 검증은 새 PlayMode `ElevationFireAimTests` 3/3 통과로 위·아래 target에 대한 3D 투사체 명중, visible Collider 접점의 Y 부호, `ShadowsOnly` 전경 건너뛰기와 수평 평면 폴백, 적 총기의 상·하 방향을 확인했다. Stage6 스모크는 NavMesh 완전 경로 5/5로 통과했다. Stage5 스모크는 새 조준 검증 단계 전 기존 씬의 무기 픽업 수 1개/기대 2개 불일치로 중단되어, 전체 Stage5 스모크 결과는 **부분 구현** 검증 상태다. 실제 마우스 조작으로 고저차 명중감과 yaw 전용 모델 시각을 보는 평가는 **확인 불가**다. 근거: `ProjectDeltatime/Assets/_Project/Tests/PlayMode/ElevationFireAimTests.cs`, `ProjectDeltatime/ElevationFireAimTests.xml`, `ProjectDeltatime/ElevationAimStage5Smoke.log`, `ProjectDeltatime/ElevationAimStage6Smoke.log`.
...
- 2026-08-17 본편 `GameHud`와 `TutorialHud`는 리플레이 장면의 시각 언어를 따르는 공용 반응형 OnGUI 전술 HUD를 사용한다. `Screen.safeArea`와 1920×1080 기준 배율, 화면 가장자리를 감싸지 않는 독립형 절단 모서리 청흑색 패널, Noto Sans KR, 낮은 채도의 청록 선을 공통 적용한다. 리플레이 전용의 화면 코너 브래킷·외곽 프레임은 인게임 HUD에서 사용하지 않는다. 청록은 기본·활성 상태, 앰버는 클리어·튜토리얼 완료·저탄약/0탄·소진 등 완료/주의 상태에만 사용한다. 좌상단에는 진행 순번 기준 `STAGE 1`·`STAGE 2`·`STAGE 3`(Stage5)·`STAGE 4`(직접 실행 Stage6) 또는 `TUTORIAL`, 좌하단에는 반복 체력 아이콘·`DEADLINE` 충전·현재 무기 실루엣·현재 탄약, 우하단에는 시계 다이얼·회전 바늘·라이브 `SCALE 0.00x` 또는 `REPLAY 1.00x`를 표시한다. 근접/빈손 탄약은 `—`로 표시하며, 중앙 클리어/사망/활성 `DEADLINE`, 본편 키캡형 조작 안내와 Tutorial 수업 안내는 보존했다. ImageGen으로 교체한 8개 아이콘은 투명 배경의 회백/암회색 기술 실루엣과 제한된 청록 표시만 사용하고 강한 흰 면·배경판·후광을 피한다. `HudIconSet`과 각 `WeaponDefinition.HudIcon` 읽기 전용 참조를 유지하며, 누락 시 텍스트/빈손 아이콘으로 대체하고 오류는 한 번만 기록한다. `HudAssetBuilder`는 알파가 실제로 존재하는 PNG인지, 임포트가 Sprite/알파 보존/밉맵 비활성/최대 256px/Clamp/Bilinear인지, 리소스와 네 무기 참조가 연결됐는지 검증한다. HUD 범위는 **구현 완료**. 외곽 프레임 제거 후 Unity 6000.1.13f1 배치 임포트·컴파일·에셋 검증과 EditMode 21/21, Stage1·Tutorial의 1920×1080/1280×720 캡처 4건이 통과했고 실제 캡처에서 화면 모서리 브래킷이 없으며 아이콘 식별성·글자 잘림·패널 비중첩·어두운 배경 대비가 유지됨을 확인했다. Prototype 전체 스모크는 기존 투척 수치/6m 착지/본 포즈 3건으로 실패했고, Tutorial 전체 스모크는 기존 Synty 인스턴스 수 216/262 검증에서 Play Mode 진입 전에 실패해 전체 스모크는 **부분 구현** 검증 상태다. Stage5·Stage6 스모크와 앰버 완료/주의 상태의 별도 캡처는 이번 변경 후 **미실행/확인 불가**다. 근거: `ProjectDeltatime/Assets/_Project/Scripts/UI/CyberHudPresentation.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/GameHud.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Tutorial/TutorialHud.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/HudIconSet.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/HudAssetBuilder.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/HudVisualCapture.cs`, `ProjectDeltatime/Assets/_Project/Tests/EditMode/HudPresentationTests.cs`, `ProjectDeltatime/Assets/_Project/Art/UI/HudIcons`.
- 2026-08-17 총기 고저차 조준은 플레이어의 수평 조준 정보와 실제 발사점을 분리한다. `PlayerAim.FireAimPoint`는 화면 포인터 Ray에서 가장 가까운 visible 비트리거 Collider 접점을 사용하고, 플레이어 자신·`ShadowsOnly` 컷어웨이 Collider는 제외하며, 대상이 없으면 플레이어 수평 평면에 폴백한다. 플레이어 총기와 적 총기는 총구에서 이 실제 높이차를 포함하는 3D 직선 방향으로 발사하지만 캐릭터/무기 모델 yaw, 반동의 수직 제한, 근접·투척의 수평 규칙은 유지한다. **구현 완료**. `ElevationFireAimTests` 3/3과 Stage6 PlayMode 스모크는 통과했고 Stage5 전체 스모크는 기존 픽업 수 불일치로 새 조준 검사 전 중단됐다. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerAim.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerCombat.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Enemies/EnemyCombatant.cs`, `ProjectDeltatime/Assets/_Project/Tests/PlayMode/ElevationFireAimTests.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/Stage5PlayModeSmokeTest.cs`.
...
- 2026-08-12 당시 저장소 루트 `AGENTS.md`와 기존 기획서·변경 기록을 작업 기준으로 검토했으며, 그 시점에는 `Assets/_Project/Tests` 폴더가 비어 있고 `.asmdef` 및 Unity Test Framework 테스트 어셈블리가 없었다. 이후 2026-08-14 리팩터링과 2026-08-17 HUD 작업으로 현재 상태는 변경됐다.
...
| 총기 사격 | 구현 완료 | 권총·샷건은 LMB Down 1회에 1회 발사하고 자동소총만 LMB 홀드 중 발사 간격마다 연사한다. 플레이어 총기 일반·`DEADLINE` 예약 발사와 적 총기는 총구에서 실제 목표 높이를 포함하는 3D 방향으로 발사하고, 투척은 기존 수평 조준을 유지한다. 성공한 매 발사는 발사 순번·펠릿 인덱스·무기 시드로 결정한 원형 콘 산포를 적용한다. 일반 플레이어 발사에서는 유효한 빈 탄약 시도도 같은 시간 활동 펄스를 발생시키지만 투사체·탄약·발사 순번은 변경하지 않는다 | `ProjectDeltatime/Assets/_Project/Scripts/Player/PlayerCombat.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Enemies/EnemyCombatant.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Combat/WeaponController.cs`, `ProjectDeltatime/Assets/_Project/Pistol.asset`, `ProjectDeltatime/Assets/_Project/AutomaticRifle.asset`, `ProjectDeltatime/Assets/_Project/Shotgun.asset` | 반동·무기 모델은 수평 yaw를 유지. 빈 자동소총 홀드는 무기 사용 간격마다만 시간 활동을 발생하며 실제 조작 체감은 확인 불가 |
...
| HUD | 구현 완료 | 공용 반응형 IMGUI 사이버 HUD가 진행 순번, 반복 체력, `DEADLINE` 충전, 현재 무기/탄약, 라이브·리플레이 시간 배율을 모서리에 표시하고 중앙 결과/활성 `DEADLINE`, 하단 조작·Tutorial 수업 안내를 보존한다 | `ProjectDeltatime/Assets/_Project/Scripts/UI/CyberHudPresentation.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/GameHud.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Tutorial/TutorialHud.cs`, `ProjectDeltatime/Assets/_Project/Resources/Hud/HudIconSet.asset` | 1920×1080·1280×720 캡처와 Safe Area/EditMode 검증 통과. Canvas/UI Toolkit 전환, 접근성·로컬라이징은 별도 과제 |
...
| 자동 테스트 | 부분 구현 | EditMode·PlayMode Unity Test Framework 어셈블리와 Prototype/Tutorial/Stage1/Stage3~Stage6 등의 커스텀 PlayMode 스모크가 있다. HUD 포맷·Safe Area·1920×1080/1280×720 레이아웃과 캡처 도구도 포함한다 | `ProjectDeltatime/Assets/_Project/Tests`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/PrototypePlayModeSmokeTest.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/TutorialPlayModeSmokeTest.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/HudVisualCapture.cs` | HUD EditMode 21/21·Stage6·캡처 4건 통과. Prototype/Tutorial/Stage5는 기존 콘텐츠 기준선 실패, 실입력 전투·클리어는 확인 불가 |
...
- HUD에 사망 리플레이와 `V`·`R` 안내가 표시된다. 사망 결과에서는 `N`을 무시한다.
...
- **현재 동작 방식:** Input System의 `Gameplay` 액션 맵을 매 프레임 폴링한다. 일반 이동은 동적 Rigidbody의 `linearVelocity`로 평면 속도를 지정하고, 사망·하드 프리즈·비활성화 시 평면 속도를 0으로 만든다. 대시는 축소한 월드 캡슐을 이동 방향으로 캐스트해 시작점이 벽에 맞닿거나 0.03 이내로 겹쳐도 안전 거리까지만 `MovePosition`한다. 조준은 수평 `AimPoint`로 플레이어 yaw·근접·투척을 유지하면서, 총기에는 카메라 포인터 Ray의 가장 가까운 visible 비트리거 접점을 별도 `FireAimPoint`로 사용한다. 적과 수직·경사 벽은 실제 접점 높이를 유지하고, 수평 바닥·천장과 유효 Collider 없는 클릭은 X/Z만 유지해 실제 총구 높이로 보정한다. 플레이어 자신과 `ShadowsOnly` 전경은 건너뛴다.
...
- **현재 동작 방식:** `WeaponController`가 현재 `WeaponKind`, 탄약과 실제/월드 시간 사용 간격을 관리한다. 플레이어 총기 일반 발사와 `DEADLINE` 준비 발사는 총구 위치에서 `PlayerAim.FireAimPoint`까지의 전체 3D 방향을 계산하며, 투척은 기존 `AimPoint`의 수평 방향을 유지한다. 적 총기도 총구에서 플레이어 루트 위치까지의 3D 방향으로 발사하고 경고선은 같은 선분을 쓴다. 모델 무기를 손에 들면 `WeaponVisualPresenter`가 모델 내부의 `Weapon Muzzle`을 만들고 `WeaponController.Muzzle`이 그 위치를 우선 사용하므로, 플레이어 탄환·적 경고선/사격 원점은 모델 총구 보정값을 따른다. 무기 모델과 캐릭터 회전은 yaw만 유지하며, 플레이어 반동도 수평 성분만 사용한다. 총기는 성공한 매 발사 때 발사 순번을 증가시키고, `WeaponSpreadPattern`이 무기 시드·발사 순번·펠릿 인덱스 기반의 상태 없는 해시로 3D 발사 방향 주위 원형 콘 안의 펠릿 방향을 결정한 뒤 투사체를 만든다. 샷건 4펠릿은 반각 9도 안에서 면적 기준으로 원형 분포하고, 패턴 전체는 발사 순번마다 결정적으로 회전한다. `Projectile`은 매 프레임 사거리 안의 SphereCast 충돌을 먼저 처리하고, 샷건은 총구에서 14m를 이동하면 명중 효과 없이 제거한다. 사거리 0m인 무기는 공용 프리팹의 4 월드초 수명만 사용한다. 네 무기의 `playerRecoilDistance`는 모두 0m이므로 실제 발사와 `DEADLINE` 해제 후에도 플레이어 위치는 반동으로 변경되지 않는다. 근접 무기는 공통 부채꼴 판정으로 시야가 확보된 가장 가까운 적대 대상 하나를 친다. 투척 무기는 장비를 즉시 해제하고 충돌 또는 최대 거리에서 픽업으로 변환된다. `worldVisualPrefab`이 정의된 무기는 투척·공중 드롭 중에도 같은 모델을 비행 루트에 표시하고, 정의되지 않은 무기만 기존 Cube를 사용한다.
...
- **현재 동작 방식:** `MainScene`은 Canvas 기반 타이틀 화면이다. Canvas는 `1920×1080` 기준 `Scale With Screen Size`·폭/높이 일치값 `0.5`를 사용하고, 배경은 원본 이미지 비율을 유지한 채 화면을 덮는다. 로고는 좌측 상단, 배경 없는 흰색 `PLAY` 텍스트는 그 아래 좌측 고정 앵커에 배치되며 투명 입력 영역을 통해 `Tutorial`을 로드한다. 게임플레이와 Tutorial은 공용 IMGUI 렌더러가 `Screen.safeArea`와 1920×1080 기준 배율로 짙은 반투명 패널·청록 포인트·밝은 실루엣 아이콘을 그린다. 좌상단 진행 순번, 좌하단 체력/`DEADLINE`과 무기/탄약, 우하단 월드 시간 또는 리플레이 배율을 고정하고, 중앙 결과/활성 `DEADLINE`, 본편 하단 조작 안내와 Tutorial 하단 수업 안내를 보존한다. 월드 시간 후처리와 `HitFlash`는 기존대로 HUD 뒤의 게임 화면에 적용된다.
- **주요 클래스:** `MainMenuController`, `CyberHudRenderer`, `GameHud`, `TutorialHud`, `HudIconSet`, `WorldTimeVisualFeedback`, `HitFlash`
- **데이터 흐름:** MainScene `PLAY` → `Tutorial`. 씬 이름/체력/`DEADLINE`/현재 무기와 탄약/`WorldTimeController.CurrentTimeScale`/리플레이 상태 → 공용 HUD. 충돌 이벤트 → `HitFlash`
- **다른 시스템과의 의존성:** Build Settings, EventSystem, Tutorial 및 거의 모든 런타임 상태 시스템
- **근거 파일:** `ProjectDeltatime/Assets/_Project/Scenes/MainScene.unity`, `ProjectDeltatime/Assets/_Project/Scripts/UI/MainMenuController.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Editor/MainSceneBuilder.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/CyberHudPresentation.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/GameHud.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Tutorial/TutorialHud.cs`, `ProjectDeltatime/Assets/_Project/Scripts/UI/HudIconSet.cs`, `ProjectDeltatime/Assets/_Project/Scripts/Time/WorldTimeVisualFeedback.cs`
- **개선이 필요한 부분:** 색약/접근성 옵션, 로컬라이징, 입력 장치별 아이콘 전환, 일시정지·설정·타이틀 복귀는 없다. 현재 IMGUI 구현을 Canvas/UI Toolkit으로 이전할지는 제품 UI 제작 단계에서 결정해야 한다.
...
- Tutorial은 공용 모서리 HUD와 한국어 단계 제목·지시·판정 진행도 패널을 함께 표시하며 완료 시 중앙 완료 패널을 표시한다.
- 좌측 상단: 활성 진행 순번 기준 `STAGE 1`·`STAGE 2`·`STAGE 3`(Stage5)·`STAGE 4`(직접 실행 Stage6), Tutorial은 `TUTORIAL`
- 좌측 하단 상단 패널: 현재/최대 체력을 채워진/어두운 반복 아이콘으로 표시하고 `DEADLINE` 아이콘 옆에 남은 충전 수 표시
- 좌측 하단 하단 패널: 현재 무기 실루엣과 현재 탄약 수. 근접 무기와 빈손은 `—` 표시
- 우측 하단: 시계 다이얼과 현재 배율에 따라 회전하는 바늘. 라이브는 `SCALE 0.00x`, 리플레이는 `REPLAY 1.00x`
- 화면 중앙: 일반 사망/클리어 메시지
...
- 적 수·실시간·대시 상태와 기존 진행 막대는 표시하지 않음
- 공용 HUD는 `Screen.safeArea`와 1920×1080 기준 배율을 사용하고, 아이콘 누락 시 HUD 전체를 끄지 않고 텍스트/범용 아이콘으로 대체함
...
- Runtime·Editor·EditMode·PlayMode 어셈블리와 Unity Test Framework 테스트가 있으나, 커스텀 스모크 일부는 현재 저장 콘텐츠와 기준값이 달라 실패한다. 사람의 실제 마우스 클릭별 조준점·총구 탄도, 애니메이션 방향·전이 체감, 튜토리얼 전체 진행 감각, 본편 전투·클리어 같은 입력 세부 조건은 직접 대조하지 않는다.
...
- 런타임·에디터·테스트 `.asmdef` 경계는 있으나 런타임 내부는 기능별 어셈블리로 더 분리하지 않았다.
- HUD는 공용 반응형 IMGUI 사이버 스타일로 정리됐지만 Canvas/UI Toolkit 기반 제품 UI, 접근성·로컬라이징·입력 장치별 아이콘은 없다.
- 사용되지 않는 것으로 확인된 시야 스텐실 머티리얼/셰이더와 생성 이미지가 남아 있다.
- `Assets/_Project/Tests`에는 EditMode·PlayMode 회귀 테스트가 있으나 실제 입력 기반 전체 게임 루프 자동화는 없다.
...
| 체력 피드백 확장 | 플레이어 HP 3을 반복 아이콘으로 표시하고 적은 원힛 사망 | 피격 무적·체력 회복·시각/음향 피드백 및 적 HP 정책 설계 | `CombatContracts.cs`, `PlayerHealth.cs`, `EnemyHealth.cs`, `CyberHudPresentation.cs` | P1 | 피해 종류와 누적 체력이 플레이·HUD·테스트에서 일관되게 확인 |
| 제품용 UI | 반응형 IMGUI 사이버 HUD와 1920×1080·1280×720/Safe Area 검증 구현 완료 | Canvas/UI Toolkit 전환 여부, 상태 우선순위 확장, 접근성·로컬라이징·입력 장치 아이콘 결정 | `CyberHudPresentation.cs`, `GameHud.cs`, `TutorialHud.cs`, `HudIconSet.cs` | P2 | 목표 플랫폼·언어·입력 장치에서 모든 상태가 접근성 기준과 함께 표시 |
...
| 테스트 구조화 | Runtime/Editor/EditMode/PlayMode asmdef와 Unity Test Framework가 있고 HUD EditMode 21/21이 통과하지만 일부 커스텀 스모크는 기존 콘텐츠 기준선과 불일치 | 실패 기준선 정리, 실제 입력·전체 루프 테스트와 CI 명령 고정 | `Assets/_Project/Tests`, `Scripts/Editor` | P2 | CI에서 단위·플레이 모드·필수 스모크를 독립 실행하고 기준선 실패가 없음 |
...
| 2026-08-17 | 1.9.4 | 플레이어의 수평 회전/근접/투척 조준과 Collider 실제 접점 기반 3D 총기 조준을 분리하고, 플레이어·적 총기와 경고선에 고저차 발사 방향을 적용 | PlayerAim, PlayerCombat, EnemyCombatant, Projectile, Stage5, PlayMode 회귀 테스트 |
| 2026-08-17 | 1.9.0 | 본편·Tutorial을 공용 반응형 사이버 HUD로 통합하고 진행 순번·반복 체력·DEADLINE·무기/탄약·시간 다이얼, ImageGen 아이콘 8종, Safe Area/해상도 테스트·캡처 검증을 반영 | GameHud, TutorialHud, HudIconSet, WeaponDefinition, HUD 에셋 빌더·테스트·캡처 |
...
- `MonoSingleton<T>`는 `Deltatime.Core`의 공용 제네릭 베이스 클래스로, 지연 접근 `Instance`, 중복 인스턴스 파괴, 종료 후 재생성 방지, `PersistAcrossScenes`(기본 `true`) 기반의 씬 전환 생존을 제공한다. 2026-08-24 기준 이를 상속하는 런타임 시스템은 없으며 기존 수동 싱글턴은 유지된다. **구현 완료**(컴파일 검증 미실행). 근거: `ProjectDeltatime/Assets/_Project/Scripts/Core/MonoSingleton.cs`.
- 에셋 감사는 삭제가 아닌 보고만 수행한다. 2026-08-14 감사 후보 15개와 Stage1 프리뷰·Stage3/4 경로 불일치는 보존 상태다. 세부 기준선과 테스트 결과는 `mdFile/REFACTORING_AUDIT.md`에 기록했다. 후보의 실제 삭제 여부는 **계획 필요**다. 근거: `ProjectDeltatime/Assets/_Project/Scripts/Editor/ProjectAssetDependencyAudit.cs`, `mdFile/REFACTORING_AUDIT.md`.

이후 기능 변경은 `mdFile/FEATURE_CHANGELOG.md`에 먼저 또는 동시에 기록하고, 이 문서의 구현 현황·시스템·수치·과제·의사결정·변경 이력을 함께 갱신한다.
