...
{
    [InitializeOnLoad]
    public static class Stage3PlayModeSmokeTest
    {
        ...
        private const string RunningKey = "Deltatime.Stage3Smoke.Running";
        private const string FailedKey = "Deltatime.Stage3Smoke.Failed";
        private const string FailureKey = "Deltatime.Stage3Smoke.Failure";
        private const string PhaseKey = "Deltatime.Stage3Smoke.Phase";
        ...
        static Stage3PlayModeSmokeTest()
        {
        ...
        private static void Tick()
        {
            ...
            {
                RecordFailure("Stage3 play-mode smoke test timed out.");
                EditorApplication.isPlaying = false;
            }
        }
        ...
        private static void ValidateRuntimeState()
        {
            ...
            Require(player != null && player.IsAlive, "Stage3 player did not initialize.");
            Require(deadline != null && deadline.ChargesRemaining == 2,
                "Stage3 Deadline charges did not initialize to 2.");
            Require(worldTime != null && worldTime.enabled,
                "Stage3 world time did not initialize.");
            Require(stage != null && stage.CurrentState == StageController.StageState.Active,
                "Stage3 did not enter the active state.");
            Require(stage != null && stage.RemainingEnemyCount == 3,
                $"Stage3 registered {stage?.RemainingEnemyCount} enemies instead of 3.");
            Require(enemies.Length == 3 && motors.Length == 3,
                "Stage3 enemy combat or navigation components are missing.");
            Require(pickups.Length == 2,
                $"Stage3 has {pickups.Length} weapon pickups instead of 2.");
            Require(replay != null && replay.enabled,
                "Stage3 replay did not initialize.");
            Require(replay != null && replay.TrackedLightCount == 2,
                $"Stage3 replay tracked {replay?.TrackedLightCount} vision lights instead of 2.");
            Require(replay != null && replay.TrackedReplayVisionConeCount == 1,
                "Stage3 replay vision-cone track did not initialize.");
            Require(surface != null && surface.navMeshData != null,
                "Stage3 NavMesh data is missing at runtime.");
            Require(
                surface != null &&
                AssetDatabase.GetAssetPath(surface.navMeshData) == NavigationPath,
                "Stage3 is not using its dedicated NavMesh data.");
            Require(GameObject.Find("Stage 3 - Afterimage Club") != null,
                "Stage3 nightclub environment root is missing.");
            Require(GameObject.Find("Club Character - Player") != null &&
                    GameObject.Find("Club Character - Bartender Gunner") != null &&
                    GameObject.Find("Club Character - Bouncer Chaser") != null &&
                    GameObject.Find("Club Character - Party Gunner") != null,
                "Stage3 Polygon Nightclubs character visuals are missing.");
        ...
        private static void RequireOnNavMesh(Vector3 position, string subject)
        {
            bool found = NavMesh.SamplePosition(
                ...
                NavMesh.AllAreas);
            Require(found,
                $"Stage3 {subject} spawn is not on the baked NavMesh ({position}).");
        }
        ...
        private static void Finish()
        {
            ...
            if (failed)
            {
                Debug.LogError("Stage3 play-mode smoke test failed:\n" + failure);
                EditorApplication.Exit(1);
                return;
            }

            Debug.Log("Stage3 play-mode smoke test passed.");
